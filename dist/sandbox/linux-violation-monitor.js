...
export function startLinuxSandboxViolationMonitor(callback, opts) {
    ...
    const shouldIgnore = (path, command) => {
        if (wildcardPaths.some(w => path.includes(w)))
            return true;
        if (command) {
    ...
    const handleEvent = (ev, encodedCommand) => {
        ...
        if (shouldIgnore(ev.path, command))
            return;
        const violation = {
